<?php
    ...
    $select="SELECT issuelog.LogID, user.UserID, issue.status, issuelog.DueDate, issue.IssueID, user.FullName, book.BookTitle, book.ISBN, book.BookImage FROM issuelog
LEFT JOIN issue On issue.IssueID=issuelog.IssueID
    ...
    WHERE user.UserID='$UserID' AND (issue.status=1 OR issue.status=2) ORDER BY issue.IssueID;";
...
<html lang="en">
    ...
    <body class="sb-nav-fixed">
        ...
        <table>
            <tr>
                <td class="float-left"><h4>My Library</h4></td>
        ...
        <thead>
            <tr>
                ...
                <th scope="col" class="text-center">Book Status</th>
        ...
        <tbody>
            ...
            <tr>
                    ...
                    <td class="text-center"><?php if ($date>$row->DueDate) {
                        echo "<label class='text-danger'>OverDue</label>";
                    } else{
                        echo "<label class='text-success'>Available</label>";
                    } ?></td>
                    <td width="15%" class="text-center">
                        <?php if ($row->status==2) {
                            echo '<a class="btn btn-secondary" href="#">Return</a>';
                        }
                        else{
                            echo '<a class="btn btn-primary" href="Return.php?IssueID='. $row->IssueID.'">Return</a>';
                        } ?>
